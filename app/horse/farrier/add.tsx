...
export default function AddFarrierEntry() {
  const router = useRouter();
  const { horseId, currentFarrier } = useLocalSearchParams();
  const { t } = useLanguage();
  ...
  const [farrierName, setFarrierName] = useState((currentFarrier as string) || '');
  ...
  function handleDateChange(val: string) {
    setDate(val);
    if (val) setNextDue(addWeeks(val, 6));
  }
  ...
  async function handleSave() {
    ...
    const [{ error: recErr }, { error: horseErr }] = await Promise.all([
      supabase.from('farrier_records').insert({
        horse_id: horseId,
        date,
    ...
    if (recErr || horseErr) {
      setError((recErr || horseErr)?.message || 'Save failed');
      setSaving(false);
      return;
    }
    router.back();
  ...
  return (
    <View style={[styles.container, { backgroundColor: C.background }]}>
      <View style={[styles.header, { backgroundColor: C.primary }]}>
        ...
        <Text style={[styles.headerTitle, { color: C.headerText, fontFamily: F.sansBold }]}>{t('Add Farrier Entry')}</Text>
        <Pressable
        ...
        <View style={[styles.section, { backgroundColor: C.card, borderColor: C.cardBorder }]}>
          ...
          {date ? (
            <View style={styles.quickFill}>
              <Text style={[styles.quickFillLabel, { color: C.textMuted, fontFamily: F.sans }]}>Quick fill:</Text>
              <Pressable
                style={({ hovered }: any) => [
                  styles.quickBtn,
              ...
              >
                <Text style={[styles.quickBtnText, { color: C.primary, fontFamily: F.sansBold }]}>+6 weeks</Text>
              </Pressable>
            </View>
          ) : null}
        </View>
