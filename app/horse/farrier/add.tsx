...
export default function AddFarrierEntry() {
  ...
  const { horseId, currentFarrier, currentShoeType } = useLocalSearchParams();
  const [date, setDate] = useState('');
  const [nextDue, setNextDue] = useState('');
  const [farrierName, setFarrierName] = useState((currentFarrier as string) || '');
  const [shoeType, setShoeType] = useState((currentShoeType as string) || '');
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Add Farrier Entry</Text>
        <Pressable
      ...
      <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.hint}>Updates the horse's default farrier info</Text>
...
const styles = StyleSheet.create({
  ...
  hint: { fontSize: 11, color: '#C4BAA8', fontStyle: 'italic', marginBottom: 12 },
